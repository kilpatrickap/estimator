...
def setup_mock_project(base_path):
    project_dir = os.path.join(base_path, "MeticulousProject_v3")
    ...
    """)
    # Item A: Plug R1. 1 unit. Bill 1500, Cost 1000. 700 Mat, 300 Lab.
    cursor.execute("INSERT INTO pboq_items VALUES ('S1', 'Item A', '1', '1500', '1000', 'R1', '', '')")
    # Item B: Gross R2. 1 unit. Bill 800, Cost 500. 500 Plant.
    cursor.execute("INSERT INTO pboq_items VALUES ('S1', 'Item B', '1', '800', '', '', '500', 'R2')")
    # Item C: Summary (SHOULD BE EXCLUDED)
    cursor.execute("INSERT INTO pboq_items VALUES ('S1', 'Subtotal Summary', '0', '2300', '', '', '', '')")
...
def run_meticulous_verification():
    print("Starting Meticulous Verification...")
    temp_dir = "temp_meticulous"
    ...
    with patch.object(FinancialExecutiveAnalytic, '_init_ui'):
        ...
        assert "2,300.00" in bid_val if 'bid_val' in locals() else "2,300.00" in bid_call
        
        # 2. Verify Cost (1000 + 500 = 1500)
