...
def debug_prov_qty(self):
    ...
    for f in os.listdir(self.pboq_folder):
        ...
        query = f"SELECT \"{d_col}\", \"{b_col}\", \"{q_col}\", \"{prov_col if prov_col else 'Sheet'}\" FROM pboq_items"
        cursor.execute(query)
        
        for row in cursor.fetchall():
            desc, b, q, prov = row
            if "structural audit" in str(desc).lower():
                print(f"DEBUG: Desc={desc} | Bid={b} | Qty={q} | Prov={prov}")
        conn.close()

debug_prov_qty(analytic)
app.quit()
