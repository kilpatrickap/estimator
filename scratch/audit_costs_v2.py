...
# Column 2 = Desc, Column 3 = Qty, [Bill Amount] = Bill, RateCode = Code
query = "SELECT [Column 2], [Column 3], [Bill Amount], RateCode FROM pboq_items WHERE [Bill Amount] != '' AND [Bill Amount] != '0.00' ORDER BY CAST(REPLACE(REPLACE([Bill Amount], ',', ''), '$', '') AS FLOAT) DESC LIMIT 10"
cursor_boq = conn_boq.cursor()
cursor_boq.execute(query)
top_items = cursor_boq.fetchall()

print(f"{'Description':<40} | {'Qty':<8} | {'Bill Amt':<12} | {'Code':<10} | {'Net(DB)':<10} | {'TotalNet':<12}")
print("-" * 110)

for desc, qty, bill_amt, code in top_items:
    ...
    qty_f = float(qty.replace(',', '')) if qty and str(qty).replace('.','').isdigit() else 0.0
    total_net = net_cost * qty_f
    
    print(f"{str(desc)[:38]:<40} | {str(qty):<8} | {str(bill_amt):<12} | {str(code):<10} | {str(net_cost):<10} | {total_net:<12.2f}")
