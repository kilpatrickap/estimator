...
*   **License Activation Dialog:** Clicking "Upgrade" opens the [LicenseActivationDialog](file:///c:/Users/Consar-Kilpatrick/Estimator_Pro_20May26/estimator/trial_splash.py#L62-L221) which displays the user's privacy-conscious **Installation Code** (e.g., `A8-C2-D9`) and provides an input field to enter and validate a timed license key (using the `EPRO-XXXXXX-XXXXXXXX-XXXXXXXX` format). On successful verification, the license details are saved to the database, granting a Green Pass.
...
8.  **Verify License Activation:** Click "Upgrade" from any failed state, note the displayed Installation Code (e.g. `A8-C2-D9`). Generate a key with `license_keygen.py` for this specific code. Enter it, verify that the validation succeeds, and confirm the app now opens immediately.
9.  **Verify Node-Locking Prevention:** Open the "Upgrade" dialog, enter a valid key generated for a **different** installation code (e.g., `B9-D3-E4`). Verify that it fails and shows `❌ This license key is registered to a different computer.`
10. **Verify Emergency Valve:** Click the "24-Hour Emergency Extension" button in Code Black (confirming it is hidden/inaccessible in Green, Yellow, and Red lanes) and confirm it temporarily grants access and launches the main window.
11. **Verify Dev Panel Reset:** After a failed roll, open the dev panel and change the override or click Reset Trial. Confirm that the percentage label resets to "0%", the bar empties, a fresh target is rolled, and the animation restarts cleanly.
